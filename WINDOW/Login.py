...
from .WindowBlueprint import Blueprint
from .Directory import ScriptDirectory
from .Registration import Registration_Interface

class Login_Interface(Blueprint): #Login Interface meaing it has no functions yet please add functions 2nd developer
  def __init__(self): #The constructor by using tk.CTk() i can call root to all of the 
    self.LF = LoginFunctions()
    self.__root = tk.CTk()
    self.SD = ScriptDirectory()
    self.Main_Window()
  ...
  def __get_show_password(self):
    self.__photo = tk.CTkImage(light_image=self.__get_image_show_password(), dark_image=self.__get_image_show_password(), size=(40, 25))
  ...
  def __get_signup(self):
    self.__sign_up = tk.CTkButton(self.__get_left_frame(), text="Sign-Up", width= 350, height= 70, font=("Poppins", 20), fg_color= "#2c2c2c", text_color="#e1e1e1", corner_radius= 50, command=lambda: self.LF.OpenRegisterWindow())
  ...
  def __get_image_logo(self):
    return Image.open(self.SD.GET_RELEVANT_PATHDIR("assets/logo.png")).convert("RGBA")

  def __get_image_show_password(self):
    return Image.open(self.SD.GET_RELEVANT_PATHDIR("assets/show_eye.png")).convert("RGBA")

  def __get_image_hide_password(self):
    return Image.open(self.SD.GET_RELEVANT_PATHDIR("assets/hide_eye.png")).convert("RGBA")
...
class LoginFunctions:
  def __init__(self):
    self.RegInt = Registration_Interface()

  def OpenRegisterWindow(self):
    self.RegInt.OpenRegistrationWindow()